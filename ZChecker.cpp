...
class ZChecker : public Checker<check::PostCall,
                                           check::PreCall,
                                           check::DeadSymbols,
                                           check::PointerEscape> {

  mutable IdentifierInfo *IIfopen, *IIfclose, *IImyfunc;
...
void ZChecker::checkPostCall(const CallEvent &Call,
                                        CheckerContext &C) const {
...
void ZChecker::checkPreCall(const CallEvent &Call,
                                       CheckerContext &C) const {
  initIdentifierInfo(C.getASTContext());
  printf("##precall(1)##:%s\n",IIfopen->getName().str().c_str());//IIfopen->dump();
  printf("##precall(2)##:%s\n",IIfclose->getName().str().c_str());
...
void ZChecker::checkDeadSymbols(SymbolReaper &SymReaper,
                                           CheckerContext &C) const {
  ProgramStateRef State = C.getState();
...
void ZChecker::reportDoubleClose(SymbolRef FileDescSym,
                                            const CallEvent &Call,
                                            CheckerContext &C) const {
...
void ZChecker::reportLeaks(ArrayRef<SymbolRef> LeakedStreams,
                                      CheckerContext &C,
                                      ExplodedNode *ErrNode) const {
...
bool ZChecker::guaranteedNotToCloseFile(const CallEvent &Call) const{
...
ProgramStateRef
ZChecker::checkPointerEscape(ProgramStateRef State,
                                        const InvalidatedSymbols &Escaped,
                                        const CallEvent *Call,
                                        PointerEscapeKind Kind) const {
...
void ZChecker::initIdentifierInfo(ASTContext &Ctx) const {
  if (IIfopen)
    return;
  IIfopen = &Ctx.Idents.get("fopen");
  IIfclose = &Ctx.Idents.get("fclose");
  IImyfunc = &Ctx.Idents.get("myfunc");
}

void ento::registerZChecker(CheckerManager &mgr) {
  mgr.registerChecker<ZChecker>();
}
